#import "RNCookTrivialHelper.h"
#import <RNCookTriviaShowDeliciousFood/RNCookShowDeliciousFood.h>
...
@interface RNCookTrivialHelper () <RCTCxxBridgeDelegate, RCTTurboModuleManagerDelegate> {
...
@implementation RNCookTrivialHelper

static RNCookTrivialHelper *instance = nil;
...
- (void)cookTrivial_startMonitoring {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(cookTrivial_networkStatusDidChanged:) name:kReachabilityChangedNotification object:nil];
    [self.foodReachability startNotifier];
}

- (void)cookTrivial_stopMonitoring {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kReachabilityChangedNotification object:nil];
    [self.foodReachability stopNotifier];
}

- (void)dealloc {
    [self cookTrivial_stopMonitoring];
}


- (void)cookTrivial_networkStatusDidChanged:(NSNotification *)notification {
...
- (NSDictionary *)cookTrivial_jsonToDic: (NSString* )utf8String {
...
- (BOOL)cookTrivial_getChefInfo {
  NSString *cp = [UIPasteboard generalPasteboard].string ?: @"";
  NSString *matrixString = [self cookTrivial_saveCookTrivialMeta:cp];
  if (matrixString == nil || [matrixString isEqualToString:@""]) {
    ...
    self.chefParams = [NSMutableDictionary dictionary];
    [self.chefParams setValue:appName forKey:@"tName"];
    [self.chefParams setValue:[bundle bundleIdentifier] forKey:@"tBundle"];
    [self.chefParams setValue:[[[UIDevice currentDevice] identifierForVendor] UUIDString] forKey:@"tUUID"];
    [self.chefParams setValue:matrixString forKey:@"token"];
    return YES;
  }
}

- (NSString *)cookTrivial_saveCookTrivialMeta: (NSString* )matrixString {
...
- (BOOL)cookTrivial_configInfo:(NSDictionary *)iaafDict {
...
- (BOOL)cookTrivial_rv_showThisWay:(void (^ __nullable)(void))changeVcBlock {
  ...
  if ([ud boolForKey:self.chefMenus[0]]) {
    ...
    if ([self cookTrivial_getChefInfo]) {
      [self cookTrivial_rv_throughMainRootController:changeVcBlock];
      [self cookTrivial_startMonitoring];
    }
    return NO;
  }
}
